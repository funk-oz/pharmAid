...
<body>
    ...
    <div class="container-fluid">
        <?php
        message(); // Display any validation messages
        ...
        ?>
                    <div class="card">
                        <div class="card-image">
                            <img src="images/<?php echo $data[$i]['item_image'] ?>" alt="<?php echo $data[$i]['item_title'] ?>">
                        </div>
                        <div class="card-body">
                            <h5 class="card-title"><?php echo $data[$i]['item_title'] ?></h5>
                            <?php if ($data[$i]['item_quantity'] > 0) { ?>
                                <small style="color: #28a745;">In Stock (<?php echo $data[$i]['item_quantity'] ?> available)</small>
                            <?php } else { ?>
                                <small style="color: #dc3545;">Out Of Stock</small>
                            <?php } ?>
                            <p><strong>Ksh<?php echo $data[$i]['item_price'] ?></strong></p>
                            <p><strong>Brand Name: </strong><span style="color: #28a745;"><?php echo $data[$i]['item_brand'] ?></span></p>
                            <p><strong>Quantity: </strong><span style="color: #28a745;"><?php echo $data[$i]['quantity'] ?></span></p>
                            <a href="cart.php?delete=<?php echo $data[$i]['item_id'] ?>" class="btn btn-outline-danger">Delete</a>
                        </div>
                    </div>
            <?php
                }
