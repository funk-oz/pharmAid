...
function all_items()
{
    $query = "SELECT i.*, c.category_name FROM item i JOIN category c ON i.category_id = c.category_id";
    $data = query($query);
    return $data;
}
...
function edit_item($id) {
    if (isset($_POST['update'])) {
        ...
        // Handle file upload
        $image = $_POST['existing_image']; // Use existing image by default
        ...
        if ($check == 0) {
            $query = "UPDATE item SET item_title='$name', item_brand='$brand', category_id='$category_id', item_details='$details', item_tags='$tags', item_image='$image', item_quantity='$quantity', item_price='$price' WHERE item_id='$id'";
            $run = single_query($query);
            get_redirect("products.php");
        } else {
...
{
    $query = "SELECT i.*, c.category_name FROM item i JOIN category c ON i.category_id = c.category_id WHERE i.item_id=$id";
    $data = query($query);
    return $data;
}
...
function search_item()
{
    if (isset($_GET['search_item'])) {
        ...
        $query = "SELECT i.*, c.category_name FROM item i JOIN category c ON i.category_id = c.category_id WHERE i.item_title LIKE '%$name%'";
        $data = query($query);
        if ($data) {
            return $data;
        } else {
...
function add_item() {
    if (isset($_POST['add_item'])) {
        ...
        $check = check_name($name);
        if (empty($name) || empty($brand) || empty($category_id) || empty($tags) || empty($image) || empty($quantity) || empty($price) || empty($details)) {
            $_SESSION['message'] = "empty_err";
            get_redirect("products.php");
            return;
        }
        if ($check == 0) {
            $query = "INSERT INTO item (item_title, item_brand, category_id, item_details, item_tags, item_image, item_quantity, item_price) VALUES ('$name', '$brand', '$category_id', '$details', '$tags', '$image', '$quantity', '$price')";
            $run = single_query($query);
            get_redirect("products.php");
        } else {
            $_SESSION['message'] = "itemErr";
