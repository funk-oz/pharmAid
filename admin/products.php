        ...
        <div class="container">
                    ...
                    <div class="form-group">
                        ...
                        <select name="category_id" class="form-control">
                            ...
                            foreach ($categories as $category) {
                                $selected = ($category['category_id'] == $data[0]['category_id']) ? 'selected' : '';
            ...
            <div class="table-responsive">
                <table class="table table-striped table-sm">
                    ...
                    <tbody>
                        <?php
                        $data = query("SELECT i.*, c.category_name FROM item i JOIN category c ON i.category_id = c.category_id");
                        delete_item();
                        if (isset($_GET['search_item'])) {
                            $query = search_item();
                            if (isset($query)) {
                                $data = query("SELECT i.*, c.category_name FROM item i JOIN category c ON i.category_id = c.category_id WHERE i.item_title LIKE '%" . trim($_GET['search_item_name']) . "%'");
                            } else {
                                get_redirect("products.php");
                            }
                        } elseif (isset($_GET['id'])) {
                            $data = query("SELECT i.*, c.category_name FROM item i JOIN category c ON i.category_id = c.category_id WHERE i.item_id = " . $_GET['id']);
                        }
                        if (isset($data)) {
                            $num = sizeof($data);
                            for ($i = 0; $i < $num; $i++) {
